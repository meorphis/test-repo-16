...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['ERIC_COMPANY_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ERIC_COMPANY_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Eric Company API.
 */
export class EricCompany {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['ERIC_COMPANY_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ERIC_COMPANY_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.EricCompanyError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the EricCompany client with an apiKey option, like new EricCompany({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? EricCompany.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ERIC_COMPANY_LOG'), "process.env['ERIC_COMPANY_LOG']", this) ??
      defaultLogLevel;
  ...
  static EricCompany = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static EricCompanyError = Errors.EricCompanyError;
...
EricCompany.Pets = Pets;
EricCompany.Store = Store;
EricCompany.UserResource = UserResource;
export declare namespace EricCompany {
