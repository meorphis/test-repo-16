...
describe('instantiate client', () => {
  ...
  describe('defaultHeaders', () => {
    const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', defaultHeaders: { 'X-My-Default-Header': '2' }, apiKey: 'My API Key' })
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['ERIC_COMPOSITIONTAR_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: EricCompositiontar) => {
  ...
  test('debug logs when log level is debug', async () => {
    ...
    const client = new EricCompositiontar({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
  ...
  test('default logLevel is warn', async () => {
    const client = new EricCompositiontar({ apiKey: 'My API Key' });
  ...
  test('debug logs are skipped when log level is info', async () => {
    ...
    const client = new EricCompositiontar({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['ERIC_COMPOSITIONTAR_LOG'] = 'debug';
    const client = new EricCompositiontar({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['ERIC_COMPOSITIONTAR_LOG'] = 'not a log level';
    const client = new EricCompositiontar({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'ERIC_COMPOSITIONTAR_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  test('client log level overrides env var', async () => {
    ...
    process.env['ERIC_COMPOSITIONTAR_LOG'] = 'debug';
    const client = new EricCompositiontar({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  describe('defaultQuery', () => {
    test('with null query params given', () => {
      const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', defaultQuery: { apiVersion: 'foo' }, apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null)).toEqual('http://localhost:5000/foo?apiVersion=foo');
    });

    test('multiple default query params', () => {
      const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', defaultQuery: { apiVersion: 'foo', hello: 'world' }, apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null)).toEqual('http://localhost:5000/foo?apiVersion=foo&hello=world');
    });

    test('overriding with `undefined`', () => {
      const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', defaultQuery: { hello: 'world' }, apiKey: 'My API Key' })
      expect(client.buildURL('/foo', { hello: undefined })).toEqual('http://localhost:5000/foo');
    });
  });

  test('custom fetch', async () => {
    const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: (url) => {
  ...
  test('explicit global fetch', async () => {
    ...
    const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: defaultFetch });
  });

  test('custom signal', async () => {
    const client = new EricCompositiontar({ baseURL: process.env["TEST_API_BASE_URL"] ?? 'http://127.0.0.1:4010', apiKey: 'My API Key', fetch: (...args) => {
  ...
  test('normalized method', async () => {
    ...
    const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['ERIC_COMPOSITIONTAR_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new EricCompositiontar({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['ERIC_COMPOSITIONTAR_BASE_URL'] = 'https://example.com/from_env';
      const client = new EricCompositiontar({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['ERIC_COMPOSITIONTAR_BASE_URL'] = ''; // empty
      const client = new EricCompositiontar({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3')
    });

    test('blank env variable', () => {
      process.env['ERIC_COMPOSITIONTAR_BASE_URL'] = '  '; // blank
      const client = new EricCompositiontar({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://petstore3.swagger.io/api/v3')
    });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new EricCompositiontar({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new EricCompositiontar({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', () => {
      const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', maxRetries: 3, apiKey: 'My API Key' });
    ...
    test('inherits options from the parent client', () => {
      const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', defaultHeaders: { 'X-Test-Header': 'test-value' }, defaultQuery: { 'test-param': 'test-value' }, apiKey: 'My API Key' });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new EricCompositiontar({ baseURL: 'http://localhost:5000/', timeout: 1000, apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    let count = 0;
      ...
      const client = new EricCompositiontar({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new EricCompositiontar({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('omit retry count header by default', async () => {
    ...
    const client = new EricCompositiontar({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4, defaultHeaders: { 'X-Stainless-Retry-Count': null } });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new EricCompositiontar({ apiKey: 'My API Key', fetch: testFetch });
