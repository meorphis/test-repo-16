# Eric Cooooooooo TypeScript API Library
...
This library provides convenient access to the Eric Cooooooooo REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCooooooooo.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCooooooooo.APIError) {
...
1. Via the `ERIC_COOOOOOOOO_LOG` environment variable
...
const client = new EricCooooooooo({
  logger: logger.child({ name: 'EricCooooooooo' }),
...
const client = new EricCooooooooo({ fetch });
```
...
```ts
import EricCooooooooo from 'npm:eric-co';
