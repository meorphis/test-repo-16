# Eric Companions! TypeScript API Library
...
This library provides convenient access to the Eric Companions REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCompanions.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCompanions.APIError) {
...
1. Via the `ERIC_COMPANIONS_LOG` environment variable
...
const client = new EricCompanions({
  logger: logger.child({ name: 'EricCompanions' }),
...
const client = new EricCompanions({ fetch });
```
...
```ts
import EricCompanions from 'npm:eric-co';
