# Eric Compositiont TypeScript API Library
...
This library provides convenient access to the Eric Compositiont REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCompositiont.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCompositiont.APIError) {
...
1. Via the `ERIC_COMPOSITIONT_LOG` environment variable
...
const client = new EricCompositiont({
  logger: logger.child({ name: 'EricCompositiont' }),
...
const client = new EricCompositiont({ fetch });
```
...
```ts
import EricCompositiont from 'npm:eric-co';
