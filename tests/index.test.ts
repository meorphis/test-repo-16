...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'ERIC_COMPOSITIONTAR_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    test('in request options overridden by env variable', () => {
      process.env['ERIC_COMPOSITIONTAR_BASE_URL'] = 'http://localhost:5000/env';
      const client = new EricCompositiontar({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
