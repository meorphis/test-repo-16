# Eric Composition! TypeScript API Library
...
This library provides convenient access to the Eric Composition REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricComposition.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricComposition.APIError) {
...
1. Via the `ERIC_COMPOSITION_LOG` environment variable
...
const client = new EricComposition({
  logger: logger.child({ name: 'EricComposition' }),
...
const client = new EricComposition({ fetch });
```
...
```ts
import EricComposition from 'npm:eric-co';
