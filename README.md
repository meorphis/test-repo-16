# Eric Companie TypeScript API Library
...
This library provides convenient access to the Eric Companie REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCompanie.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCompanie.APIError) {
...
1. Via the `ERIC_COMPANIE_LOG` environment variable
...
const client = new EricCompanie({
  logger: logger.child({ name: 'EricCompanie' }),
...
const client = new EricCompanie({ fetch });
```
...
```ts
import EricCompanie from 'npm:eric-co';
