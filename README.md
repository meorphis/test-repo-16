# Eric Cooo TypeScript API Library
...
This library provides convenient access to the Eric Cooo REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCooo.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCooo.APIError) {
...
1. Via the `ERIC_COOO_LOG` environment variable
...
const client = new EricCooo({
  logger: logger.child({ name: 'EricCooo' }),
...
const client = new EricCooo({ fetch });
```
...
```ts
import EricCooo from 'npm:eric-co';
