...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['ERIC_COMP_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ERIC_COMP_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Eric Comp API.
 */
export class EricComp {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['ERIC_COMP_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ERIC_COMP_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.EricCompError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the EricComp client with an apiKey option, like new EricComp({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? EricComp.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ERIC_COMP_LOG'), "process.env['ERIC_COMP_LOG']", this) ??
      defaultLogLevel;
  ...
  static EricComp = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static EricCompError = Errors.EricCompError;
...
EricComp.Pets = Pets;
EricComp.Store = Store;
EricComp.UserResource = UserResource;
export declare namespace EricComp {
