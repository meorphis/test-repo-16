# Eric Companies! TypeScript API Library
...
This library provides convenient access to the Eric Companies REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCompanies.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCompanies.APIError) {
...
1. Via the `ERIC_COMPANIES_LOG` environment variable
...
const client = new EricCompanies({
  logger: logger.child({ name: 'EricCompanies' }),
...
const client = new EricCompanies({ fetch });
```
...
```ts
import EricCompanies from 'npm:eric-co';
