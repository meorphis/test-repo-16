# Eric Coooo TypeScript API Library!
...
This library provides convenient access to the Eric Coooo REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCoooo.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCoooo.APIError) {
...
1. Via the `ERIC_COOOO_LOG` environment variable
...
const client = new EricCoooo({
  logger: logger.child({ name: 'EricCoooo' }),
...
const client = new EricCoooo({ fetch });
```
...
```ts
import EricCoooo from 'npm:eric-co';
