...
This library provides convenient access to the Eric Coo REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCoo.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCoo.APIError) {
...
1. Via the `ERIC_COO_LOG` environment variable
...
const client = new EricCoo({
  logger: logger.child({ name: 'EricCoo' }),
...
const client = new EricCoo({ fetch });
```
...
```ts
import EricCoo from 'npm:eric-co';
