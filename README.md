# Eric Coooooo TypeScript API Library
...
This library provides convenient access to the Eric Coooooo REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCoooooo.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCoooooo.APIError) {
...
1. Via the `ERIC_COOOOOO_LOG` environment variable
...
const client = new EricCoooooo({
  logger: logger.child({ name: 'EricCoooooo' }),
...
const client = new EricCoooooo({ fetch });
```
...
```ts
import EricCoooooo from 'npm:eric-co';
