# Eric Cooooooo TypeScript API Library
...
This library provides convenient access to the Eric Cooooooo REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCooooooo.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCooooooo.APIError) {
...
1. Via the `ERIC_COOOOOOO_LOG` environment variable
...
const client = new EricCooooooo({
  logger: logger.child({ name: 'EricCooooooo' }),
...
const client = new EricCooooooo({ fetch });
```
...
```ts
import EricCooooooo from 'npm:eric-co';
