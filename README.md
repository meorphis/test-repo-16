# Eric Compositiona! TypeScript API Library
...
This library provides convenient access to the Eric Compositiona REST API from server-side TypeScript or JavaScript.

The REST API documentation can be found on [docs.eric-cooooo.com](https://docs.eric-cooooo.com). The full API of this library can be found in [api.md](api.md).
...
async function main() {
  const response: EricCompositiona.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCompositiona.APIError) {
...
1. Via the `ERIC_COMPOSITIONA_LOG` environment variable
...
const client = new EricCompositiona({
  logger: logger.child({ name: 'EricCompositiona' }),
...
const client = new EricCompositiona({ fetch });
```
...
```ts
import EricCompositiona from 'npm:eric-co';
