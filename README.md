# Eric Compositioning! TypeScript API Library
...
This library provides convenient access to the Eric Compositioning REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCompositioning.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCompositioning.APIError) {
...
1. Via the `ERIC_COMPOSITIONING_LOG` environment variable
...
const client = new EricCompositioning({
  logger: logger.child({ name: 'EricCompositioning' }),
...
const client = new EricCompositioning({ fetch });
```
...
```ts
import EricCompositioning from 'npm:eric-co';
