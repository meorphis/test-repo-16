# Eric Coooooooo TypeScript API Library
...
This library provides convenient access to the Eric Coooooooo REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCoooooooo.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCoooooooo.APIError) {
...
1. Via the `ERIC_COOOOOOOO_LOG` environment variable
...
const client = new EricCoooooooo({
  logger: logger.child({ name: 'EricCoooooooo' }),
...
const client = new EricCoooooooo({ fetch });
```
...
```ts
import EricCoooooooo from 'npm:eric-co';
