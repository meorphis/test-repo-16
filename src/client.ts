...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['ERIC_COMPANIE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ERIC_COMPANIE_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Eric Companie API.
 */
export class EricCompanie {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['ERIC_COMPANIE_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ERIC_COMPANIE_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.EricCompanieError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the EricCompanie client with an apiKey option, like new EricCompanie({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? EricCompanie.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('ERIC_COMPANIE_LOG'), "process.env['ERIC_COMPANIE_LOG']", this) ??
      defaultLogLevel;
  ...
  static EricCompanie = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static EricCompanieError = Errors.EricCompanieError;
...
EricCompanie.Pets = Pets;
EricCompanie.Store = Store;
EricCompanie.UserResource = UserResource;
export declare namespace EricCompanie {
