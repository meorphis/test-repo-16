# Eric Coooooooooo TypeScript API Library
...
This library provides convenient access to the Eric Coooooooooo REST API from server-side TypeScript or JavaScript.
...
async function main() {
  const response: EricCoooooooooo.StoreInventoryResponse = await client.store.inventory();
}
...
async function main() {
  const response = await client.store.inventory().catch(async (err) => {
    if (err instanceof EricCoooooooooo.APIError) {
...
1. Via the `ERIC_COOOOOOOOOO_LOG` environment variable
...
const client = new EricCoooooooooo({
  logger: logger.child({ name: 'EricCoooooooooo' }),
...
const client = new EricCoooooooooo({ fetch });
```
...
```ts
import EricCoooooooooo from 'npm:eric-co';
